...
# custom
from utils import evaluate_metrics, vis_history

# hyperparameters
th = 0.4  # 기준값을 0.5로 설정
n_epochs = 200
x_shape = (1000, 4)  # 12-lead + rithmy throad
k_folds = 5
...
def train_cls(base):
    ...
    for lr in [0.000005]:
        ...
        for train_index, val_index in kfold.split(X_train):
            ...
            clf = InceptionTimeClassifier(verbose=True, kernel_size=20, n_filters=32, use_residual=False, bottleneck_size=16, random_state=0).build_model(
            ...
            # Training the model
            history = clf.fit(X_train_fold, y_train_fold, validation_data=(X_val_fold, y_val_fold), epochs=n_epochs, batch_size=32, shuffle=True)
