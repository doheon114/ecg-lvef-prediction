...
def train_reg():
    ...
    for lr in [0.0005]:
        clf = InceptionTimeRegressor(n_filters=16,        # 필터 수 조정
                                                                      
                                    #kernel_size=40,       # 커널 크기
                                    batch_size=128,
                                    verbose=True,
                                    random_state=0
                                    ).build_model(input_shape=(1000, 4))
        clf.compile(optimizer=Adam(learning_rate=lr), loss="mean_squared_error", metrics=["mean_absolute_error"])

        history = clf.fit(X_train, y_train, validation_split=0.1, epochs=100, callbacks=[tensorboard_callback], batch_size=128)
        ...
        for X, y, dataset in [(X_int, y_int, "int"), (X_ext, y_ext, "ext")]:
            ...
            plt.tight_layout()
            plt.savefig(f"/home/work/.LVEF/ecg-lvef-prediction/results/reg/{dataset}_{lr}.png")
        # TensorBoard 실행
