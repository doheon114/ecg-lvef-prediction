...
# 필요 시 데이터 타입 변환
X_int = X_int.astype(np.float32)  # 또는 np.int32
y_int_binary = y_int_binary.astype(np.int32)  # 클래스 레이블의 경우

# 필요 시 데이터 타입 변환
X_ext = X_ext.astype(np.float32)  # 또는 np.int32
y_ext_binary = y_ext_binary.astype(np.int32)  # 클래스 레이블의 경우
...
def train_cls_with_ensemble(base):
    ...
    for lr in [0.000005]:
        ...
        for train_index, val_index in kfold.split(X_train):
            ...
            # 필요 시 데이터 타입 변환
            X_train_fold = X_train_fold.astype(np.float32)  # 또는 np.int32
            y_train_fold = y_train_fold.astype(np.int32)  # 클래스 레이블의 경우

            # 필요 시 데이터 타입 변환
            X_val_fold = X_val_fold.astype(np.float32)  # 또는 np.int32
            y_val_fold = y_val_fold.astype(np.int32)  # 클래스 레이블의 경우
            ...
            # 두 개의 GPU를 사용하도록 MirroredStrategy 설정
            ...
            with strategy.scope():
                ...
                clf = InceptionTimeClassifier(verbose=True,
                                            ...
                                            depth=depth, 
                                            random_state=0,
                                            ).build_model(input_shape=(5000, 12), n_classes=2)
                # clf = ResNetClassifier().build_model(input_shape=(5000, 12), n_classes=2)
